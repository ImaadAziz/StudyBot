...
class Poll {
  ...
  async finish(client) {
    ...
    if (!message) {
      console.error("Cannot find poll message");
      return;
    }
  ...
  generateEmbed() {
    ...
    if (this.isTimed)
      footer += ` | This poll ends in ${new Date(
        this.finishTime
      ).toUTCString()}`;
  ...
  generateResultsEmbed() {
    ...
    if (this.type !== "yn") {
      // only sort if its not a yn poll
    ...
    finalResults.forEach((result) => {
      description += `${result.emoji} ${result.answer} :: ** ${result.votes} ** :: ${result.percentage}% \n`;
    });

    let footer = `Results from poll ${this.id} finished on ${new Date(
      this.finishTime
    ).toUTCString()}`;
